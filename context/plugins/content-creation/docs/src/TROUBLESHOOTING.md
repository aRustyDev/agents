...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin content-creation` |
| MCP server timeout | Server not running | Run `just enable-mcp content-creation` |
...
2. Install plugin dependencies: `cd context/plugins/content-creation && brew bundle`
3. Retry installation: `just install-plugin content-creation`
...
2. Restart the server: `just restart-mcp content-creation`
...
A: Run `just update-plugin content-creation` to fetch the latest version.
