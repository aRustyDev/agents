...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin infrastructure` |
| MCP server timeout | Server not running | Run `just enable-mcp infrastructure` |
...
2. Install plugin dependencies: `cd context/plugins/infrastructure && brew bundle`
3. Retry installation: `just install-plugin infrastructure`
...
2. Restart the server: `just restart-mcp infrastructure`
...
A: Run `just update-plugin infrastructure` to fetch the latest version.
