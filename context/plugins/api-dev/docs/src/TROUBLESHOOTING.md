...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin api-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp api-dev` |
...
2. Install plugin dependencies: `cd context/plugins/api-dev && brew bundle`
3. Retry installation: `just install-plugin api-dev`
...
2. Restart the server: `just restart-mcp api-dev`
...
A: Run `just update-plugin api-dev` to fetch the latest version.
