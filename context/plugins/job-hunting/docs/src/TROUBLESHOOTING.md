...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin job-hunting` |
| MCP server timeout | Server not running | Run `just enable-mcp job-hunting` |
...
2. Install plugin dependencies: `cd context/plugins/job-hunting && brew bundle`
3. Retry installation: `just install-plugin job-hunting`
...
2. Restart the server: `just restart-mcp job-hunting`
...
A: Run `just update-plugin job-hunting` to fetch the latest version.
