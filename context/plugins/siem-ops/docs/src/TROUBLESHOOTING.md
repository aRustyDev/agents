...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin siem-ops` |
| MCP server timeout | Server not running | Run `just enable-mcp siem-ops` |
...
2. Install plugin dependencies: `cd context/plugins/siem-ops && brew bundle`
3. Retry installation: `just install-plugin siem-ops`
...
2. Restart the server: `just restart-mcp siem-ops`
...
A: Run `just update-plugin siem-ops` to fetch the latest version.
