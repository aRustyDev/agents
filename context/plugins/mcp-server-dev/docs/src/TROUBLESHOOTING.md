...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin mcp-server-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp mcp-server-dev` |
...
2. Install plugin dependencies: `cd context/plugins/mcp-server-dev && brew bundle`
3. Retry installation: `just install-plugin mcp-server-dev`
...
2. Restart the server: `just restart-mcp mcp-server-dev`
...
A: Run `just update-plugin mcp-server-dev` to fetch the latest version.
