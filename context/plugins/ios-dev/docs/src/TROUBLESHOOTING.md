...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin ios-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp ios-dev` |
...
2. Install plugin dependencies: `cd context/plugins/ios-dev && brew bundle`
3. Retry installation: `just install-plugin ios-dev`
...
2. Restart the server: `just restart-mcp ios-dev`
...
A: Run `just update-plugin ios-dev` to fetch the latest version.
