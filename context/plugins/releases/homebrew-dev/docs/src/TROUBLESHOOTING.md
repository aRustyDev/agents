...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin homebrew-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp homebrew-dev` |
...
2. Install plugin dependencies: `cd context/plugins/homebrew-dev && brew bundle`
3. Retry installation: `just install-plugin homebrew-dev`
...
2. Restart the server: `just restart-mcp homebrew-dev`
...
A: Run `just update-plugin homebrew-dev` to fetch the latest version.
