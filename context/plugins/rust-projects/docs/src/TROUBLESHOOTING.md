...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin rust-projects` |
| MCP server timeout | Server not running | Run `just enable-mcp rust-projects` |
...
2. Install plugin dependencies: `cd context/plugins/rust-projects && brew bundle`
3. Retry installation: `just install-plugin rust-projects`
...
2. Restart the server: `just restart-mcp rust-projects`
...
A: Run `just update-plugin rust-projects` to fetch the latest version.
