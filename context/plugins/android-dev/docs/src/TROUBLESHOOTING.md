...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin android-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp android-dev` |
...
2. Install plugin dependencies: `cd context/plugins/android-dev && brew bundle`
3. Retry installation: `just install-plugin android-dev`
...
2. Restart the server: `just restart-mcp android-dev`
...
A: Run `just update-plugin android-dev` to fetch the latest version.
