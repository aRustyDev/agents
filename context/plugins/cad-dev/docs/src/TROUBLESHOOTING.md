...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin cad-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp cad-dev` |
...
2. Install plugin dependencies: `cd context/plugins/cad-dev && brew bundle`
3. Retry installation: `just install-plugin cad-dev`
...
2. Restart the server: `just restart-mcp cad-dev`
...
A: Run `just update-plugin cad-dev` to fetch the latest version.
