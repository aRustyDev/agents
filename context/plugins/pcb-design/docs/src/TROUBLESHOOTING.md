...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin pcb-design` |
| MCP server timeout | Server not running | Run `just enable-mcp pcb-design` |
...
2. Install plugin dependencies: `cd context/plugins/pcb-design && brew bundle`
3. Retry installation: `just install-plugin pcb-design`
...
2. Restart the server: `just restart-mcp pcb-design`
...
A: Run `just update-plugin pcb-design` to fetch the latest version.
