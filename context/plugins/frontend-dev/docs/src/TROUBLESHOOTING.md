...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin frontend-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp frontend-dev` |
...
2. Install plugin dependencies: `cd context/plugins/frontend-dev && brew bundle`
3. Retry installation: `just install-plugin frontend-dev`
...
2. Restart the server: `just restart-mcp frontend-dev`
...
A: Run `just update-plugin frontend-dev` to fetch the latest version.
