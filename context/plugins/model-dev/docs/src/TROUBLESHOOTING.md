...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin model-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp model-dev` |
...
2. Install plugin dependencies: `cd context/plugins/model-dev && brew bundle`
3. Retry installation: `just install-plugin model-dev`
...
2. Restart the server: `just restart-mcp model-dev`
...
A: Run `just update-plugin model-dev` to fetch the latest version.
