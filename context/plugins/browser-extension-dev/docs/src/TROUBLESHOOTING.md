...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin browser-extension-dev` |
| MCP server timeout | Server not running | Run `just enable-mcp browser-extension-dev` |
...
2. Install plugin dependencies: `cd context/plugins/browser-extension-dev && brew bundle`
3. Retry installation: `just install-plugin browser-extension-dev`
...
2. Restart the server: `just restart-mcp browser-extension-dev`
...
A: Run `just update-plugin browser-extension-dev` to fetch the latest version.
