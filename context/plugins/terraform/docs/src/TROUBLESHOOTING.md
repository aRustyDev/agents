...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin terraform` |
| MCP server timeout | Server not running | Run `just enable-mcp terraform` |
...
2. Install plugin dependencies: `cd context/plugins/terraform && brew bundle`
3. Retry installation: `just install-plugin terraform`
...
2. Restart the server: `just restart-mcp terraform`
...
A: Run `just update-plugin terraform` to fetch the latest version.
