...
import type { BackfillResult, CatalogEntry, CatalogEntryWithTier1, Tier1ErrorType } from './types'
...
// ---------------------------------------------------------------------------
// skill-discovery dynamic import
// ---------------------------------------------------------------------------

/**
 * Dynamically import discoverSkills from the CLI package.
 * This avoids a static dependency on skill-discovery.ts which
 * remains in CLI until Phase 5.
 */
async function loadDiscoverSkills(): Promise<
  (
    dir: string
  ) => Promise<
    import('@agents/core/types').Result<
      Array<{ path: string; name: string; frontmatter: { description?: string } }>
    >
  >
> {
  try {
...
async function findSkillInDir(
  ...
  const discoverSkills = await loadDiscoverSkills()
...
export async function downloadSkill(
  ...
  if (!validated.ok) {
    return {
      ...
      errorDetail: validated.error.detail ?? validated.error.message,
    }
  }
  ...
  if (!resolved.ok) {
    ...
    return {
      ...
      errorDetail: resolved.error.detail ?? resolved.error.message,
    }
  }
