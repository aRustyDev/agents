...
import { type PluginSourceExtended, PluginSourcesManifest } from '../../context/plugin/schema'
 ...
 */
export async function readLockfile<T>(schemaName: string, path: string): Promise<Result<T>> {
...
export async function writeLockfile(
  ...
  if (!result.success) {
    const issues = result.issues
      ...
      .join('\n')
    return err(
      new SdkError(`Data does not match schema "${schemaName}"`, 'E_VALIDATION_FAILED', issues)
    )
  }
