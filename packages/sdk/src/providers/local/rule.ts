...
export class LocalRuleProvider implements ComponentProvider {
  ...
  async add(_source: string, _opts: ComponentAddOptions): Promise<Result<ComponentAddResult>> {
    return err(new SdkError('Rule provider does not support add', 'E_PROVIDER_UNAVAILABLE'))
  }
  ...
  async remove(
    ...
    return err(new SdkError('Rule provider does not support remove', 'E_PROVIDER_UNAVAILABLE'))
  }
  ...
  async info(
    ...
    if (!SUPPORTED_TYPES.includes(type)) {
      return err(
        new SdkError(`Rule provider does not support info for ${type}`, 'E_PROVIDER_UNAVAILABLE')
      )
    }
    ...
    if (!found) {
      return err(new SdkError(`Rule not found: "${name}"`, 'E_COMPONENT_NOT_FOUND'))
    }
