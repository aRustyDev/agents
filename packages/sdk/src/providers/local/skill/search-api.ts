 ...
 *   2. meilisearch -- local Meilisearch instance (via @agents/kg/meilisearch)
...
// ---------------------------------------------------------------------------
// Backend: Meilisearch (dynamic import to avoid hard SDK -> KG dependency)
// ---------------------------------------------------------------------------

async function searchMeili(query: string, limit: number): Promise<Result<SkillSearchResult[]>> {
  try {
    // Dynamic import to keep SDK -> KG dependency optional
    const { createClient, checkHealth, searchKeyword } = await import('@agents/kg/meilisearch')
