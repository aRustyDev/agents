 ...
 *
 * @param resolver - Optional AgentResolver for detecting installed agents.
 * @param source - Source string (local path, GitHub shorthand, URL, etc.).
...
export async function addSkill(
  ...
  try {
    ...
    const { discoverSkills, filterSkills } = await import('../../../context/skill/discovery')
    ...
    if (targetAgents.length === 0 && resolver) {
      const installedConfigs = resolver.detectInstalled()
      targetAgents = installedConfigs.map((c) => c.name)
    ...
    for (const skill of skills) {
      ...
      for (const agentName of targetAgents) {
        const baseDir = resolver?.getBaseDir(agentName, false, cwd)
        if (!baseDir) {
          warnings.push(`Could not resolve skills dir for agent "${agentName}"`)
          continue
        }

        const agentSkillDir = join(baseDir, skillName)
