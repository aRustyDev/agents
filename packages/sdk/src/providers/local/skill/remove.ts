 ...
 *
 * @param resolver - Optional AgentResolver for looking up agent directories.
 * @param names - Skill names to remove.
...
export async function removeSkills(
  ...
  const allAgents: AgentConfig[] = resolver?.list() ?? []

  // Determine target agents for removal
  const targetAgents: AgentConfig[] = opts.agent
    ? allAgents.filter((a) => a.name === opts.agent)
    : allAgents
  ...
  for (const name of names) {
    ...
    for (const agent of targetAgents) {
      const baseDir = resolver?.getBaseDir(agent.name, opts.global ?? false, cwd)
      if (!baseDir) continue

      const agentSkillPath = join(baseDir, name)
      if (!existsSync(agentSkillPath)) continue

      try {
        await rm(agentSkillPath, { recursive: true, force: true })
        removedFrom.push(agent.name)
      } catch {
    ...
    if (opts.agent) {
      ...
      for (const agent of allAgents) {
        if (agent.name === opts.agent) continue
        const baseDir = resolver?.getBaseDir(agent.name, opts.global ?? false, cwd)
        if (!baseDir) continue
