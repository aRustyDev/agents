 ...
 *
 * Supports `--agent` and `--skill` filters via the shared `filters` module.
 *
 ...
 *
 * @param resolver - Optional AgentResolver for detecting which agents have each skill.
 * @param opts - List options (cwd, filters, etc.).
 */
export async function listSkills(
  ...
  const agents: AgentConfig[] = resolver?.list() ?? []
  ...
  for (const entry of entries) {
    ...
    // Parse frontmatter (dynamic import to avoid circular deps)
    const { readSkillFrontmatter } = await import('../../../context/manifest')
    ...
    const agentList = detectAgentsForSkill(entry.name, cwd, agents)
 ...
 */
function detectAgentsForSkill(skillName: string, cwd: string, agents: AgentConfig[]): string[] {
  const found: string[] = []
