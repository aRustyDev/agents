...
import type { PublishResult } from '../../context/types'
...
export async function publishToSmithery(
  ...
  return err(
    new SdkError('Either externalUrl or bundleDir must be provided', 'E_VALIDATION_FAILED')
  )
}
...
async function publishBundle(
  ...
  try {
    ...
    if (response.status === 404) {
      return err(new SdkError(`Server "${opts.qualifiedName}" not found`, 'E_COMPONENT_NOT_FOUND'))
    }

    if (response.status === 401 || response.status === 403) {
      return err(new SdkError('Authentication failed', 'E_PROVIDER_UNAVAILABLE'))
    }

    if (!response.ok) {
      return err(
        new SdkError(`Smithery API error: HTTP ${response.status}`, 'E_PROVIDER_UNAVAILABLE')
      )
    }
  ...
  while (Date.now() - start < timeout) {
    try {
      ...
      if (!response.ok) {
        return err(
          new SdkError(
            `Failed to poll deployment: HTTP ${response.status}`,
            'E_PROVIDER_UNAVAILABLE'
          )
        )
      }
