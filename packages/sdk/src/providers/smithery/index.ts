...
export class SmitheryProvider implements ComponentProvider {
  ...
  async add(_source: string, _opts: ComponentAddOptions): Promise<Result<ComponentAddResult>> {
    return err(new SdkError('Smithery provider does not support add', 'E_PROVIDER_UNAVAILABLE'))
  }
  ...
  async remove(
    ...
    return err(new SdkError('Smithery provider does not support remove', 'E_PROVIDER_UNAVAILABLE'))
  }
  ...
  async info(
    ...
    return err(new SdkError('Smithery provider does not support info', 'E_PROVIDER_UNAVAILABLE'))
  }
  ...
  async publish(type: ComponentType, opts: PublishOptions): Promise<Result<PublishResult>> {
    if (type !== 'mcp-server') {
      return err(
        new SdkError(
          `Smithery only supports publishing mcp-server, not ${type}`,
    ...
    // Resolve auth
    const { resolveSmitheryAuth } = await import('./auth')
    ...
    if (!qualifiedName) {
      return err(
        new SdkError('Qualified name required (--name namespace/server)', 'E_VALIDATION_FAILED')
      )
    }

    // Delegate to publish module
    const { publishToSmithery } = await import('./publish')
