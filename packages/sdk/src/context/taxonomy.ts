...
// ---------------------------------------------------------------------------
// Entry point (parameterized — accepts projectRoot instead of using currentDir)
// ---------------------------------------------------------------------------
...
export function runTaxonomy(opts: RunTaxonomyOptions): void {
  const todoPath = path.join(opts.projectRoot, 'content/skills/.TODO.yaml')
  const taxonomyPath = path.join(opts.projectRoot, 'content/skills/.taxonomy.yaml')

  if (!fs.existsSync(todoPath)) {
    console.error(`Error: .TODO.yaml not found at ${todoPath}`)
    console.error('Generate it first with: uv run python cli/generate-todo.py')
    process.exit(1)
  }

  console.log('Loading .TODO.yaml...')
  const rawContent = fs.readFileSync(todoPath, 'utf-8')
  ...
  console.log(`\nTaxonomy written to: ${taxonomyPath}`)
}
