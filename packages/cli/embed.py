...
ENTITY_PATTERNS = {
    ...
    "rule": ["content/rules/*.md", "content/rules/**/*.md", ".claude/rules/*.md"],
    "claude_md": ["**/CLAUDE.md"],
...
def ingest_file(
    ...
    existing = conn.execute("SELECT id, file_hash FROM entities WHERE slug = ?", (slug,)).fetchone()
...
def cmd_search(args):
    ...
    results = conn.execute(
        ...
        LIMIT ?
    """,
        (serialize_embedding(query_embedding), args.limit),
    ).fetchall()
...
def cmd_similarity(args):
    ...
    if args.entity:
        ...
        entity = conn.execute("SELECT id FROM entities WHERE slug = ?", (args.entity,)).fetchone()
...
def cmd_similar(args):
    ...
    # Look up entity
    entity = conn.execute(
        "SELECT id, name, entity_type FROM entities WHERE slug = ?", (args.entity,)
    ).fetchone()
...
def main():
    ...
    p_rebuild = subparsers.add_parser(
        "rebuild-embeddings", help="Rebuild vec_chunks from existing chunks"
    )
