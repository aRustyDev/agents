 ...
 *
 * The interactive search path (live-as-you-type with `searchMultiselect`) is
 * deferred -- `searchMultiselect` currently accepts a static `items` array
 * and does not support a dynamic search callback. For now, interactive mode
 * prints usage hints.
 */

import { createOutput, type OutputFormatter } from '@agents/core/output'
