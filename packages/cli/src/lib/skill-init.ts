 ...
 * Matches the contents of `packages/cli/templates/SKILL.md.tmpl`.
 */
 ...
 * 2. Project default at `packages/cli/templates/SKILL.md.tmpl`
...
async function loadTemplate(templatePath: string | undefined, cwd: string): Promise<string> {
  ...
  const projectTemplate = join(cwd, 'packages', 'cli', 'templates', 'SKILL.md.tmpl')
