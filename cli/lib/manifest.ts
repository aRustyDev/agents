...
export async function readPluginManifest(dir: string): Promise<Result<PluginManifest>> {
  ...
  try {
    ...
    return err(
      new CliError(`Failed to read plugin manifest: ${manifestPath}`, 'E_READ_FAILED', undefined, e)
    )
  }
...
export async function readPluginSources(dir: string): Promise<Result<PluginSourcesManifest>> {
  ...
  try {
    ...
    return err(
      new CliError(`Failed to read plugin sources: ${sourcesPath}`, 'E_READ_FAILED', undefined, e)
    )
  }
...
export async function readSkillFrontmatter(skillPath: string): Promise<Result<SkillFrontmatter>> {
  ...
  try {
    ...
    return err(
      new CliError(`Failed to read skill file: ${skillPath}`, 'E_READ_FAILED', undefined, e)
    )
  }
...
export function normalizeSource(entry: unknown): Result<ExtendedSource> {
  ...
  switch (format) {
    case 'legacy': {
      if (typeof entry !== 'string') {
        return err(new CliError('Expected a string for legacy source format', 'E_INVALID_SOURCE'))
      }
