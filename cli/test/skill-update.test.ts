...
import type { AddResult } from '../lib/skill-add'
import type { OutdatedResult } from '../lib/skill-outdated'
...
function addFailed(message: string): AddResult {
  const { CliError } = require('../lib/types') as typeof import('../lib/types')
  return {
    ok: false,
    installed: [],
...
describe('UpdateError', () => {
  test('has skill, message, and code fields', async () => {
    const { UpdateError } = await import('../lib/skill-update')
