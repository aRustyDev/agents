...
export class LocalAgentProvider implements ComponentProvider {
  ...
  private async parseAgentFile(filePath: string): Promise<Component | null> {
    ...
    const name =
      typeof meta.name === 'string' && meta.name.length > 0 ? meta.name : basename(filePath, '.md')

    const description = typeof meta.description === 'string' ? meta.description : ''

    // Parse tools: comma-separated string -> string[]
    let tags: string[] | undefined
    if (typeof meta.tools === 'string' && meta.tools.length > 0) {
      tags = meta.tools
        .split(', ')
        .map((t: string) => t.trim())
        .filter(Boolean)
  ...
  async info(
    ...
    if (!SUPPORTED_TYPES.includes(type)) {
      return err(
        new CliError(`Local agent provider does not support info for ${type}`, 'E_UNSUPPORTED_TYPE')
      )
    }
