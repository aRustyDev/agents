 ...
 * Matches the contents of `.scripts/templates/SKILL.md.tmpl`.
 */
 ...
 * 2. Project default at `.scripts/templates/SKILL.md.tmpl`
...
async function loadTemplate(templatePath: string | undefined, cwd: string): Promise<string> {
  ...
  const projectTemplate = join(cwd, '.scripts', 'templates', 'SKILL.md.tmpl')
