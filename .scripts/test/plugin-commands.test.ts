...
describe('status derivation logic', () => {
  ...
  test('forked takes priority', () => {
    expect(deriveStatus({ forked: true, missing: true, expectedHash: 'abc', actualHash: 'def' }))
      .toBe('forked')
  })

  test('missing takes priority over hash checks', () => {
    expect(deriveStatus({ forked: false, missing: true, expectedHash: 'abc', actualHash: null }))
      .toBe('missing')
  })

  test('no-hash when expected is null', () => {
    expect(deriveStatus({ forked: false, missing: false, expectedHash: null, actualHash: 'abc' }))
      .toBe('no-hash')
  })

  test('fresh when hashes match', () => {
    expect(deriveStatus({ forked: false, missing: false, expectedHash: 'abc', actualHash: 'abc' }))
      .toBe('fresh')
  })

  test('stale when hashes differ', () => {
    expect(deriveStatus({ forked: false, missing: false, expectedHash: 'abc', actualHash: 'def' }))
      .toBe('stale')
  })
})
