...
describe('checkSymlink', () => {
  ...
  test('throws for a regular file (not a symlink)', async () => {
    expect(checkSymlink(join(root, 'real-file.txt'))).rejects.toThrow(
      'Not a symlink',
    )
  })
...
describe('createSymlink', () => {
  ...
  test('throws when path exists and is not a symlink', async () => {
    expect(
      createSymlink(join(root, 'real-dir'), join(root, 'real-file.txt')),
    ).rejects.toThrow('Path exists and is not a symlink')
  })
})
