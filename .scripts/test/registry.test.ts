...
describe('validateNdjson', () => {
  ...
  test('validates valid NDJSON records', () => {
    ...
    const records = [
      { id: 'test-1', name: 'Test One', type: 'skill', source_name: 'test', source_type: 'registry' },
      { id: 'test-2', name: 'Test Two', type: 'plugin', source_name: 'github', source_type: 'github' },
    ]
    writeFileSync(
      filePath,
  ...
  test('skips blank lines', () => {
    ...
    const record = { id: 'test', name: 'Test', type: 'skill', source_name: 'x', source_type: 'registry' }
    writeFileSync(
      filePath,
...
describe('parseAwesomeReadme', () => {
  ...
  test('skips anchor links', () => {
    const md = [
      '- [Section](#section)',
      '- [Tool](https://github.com/org/tool) - Real',
    ].join('\n')
...
describe('registry command module', () => {
  test('exports a default defineCommand', async () => {
    const mod = await import('../commands/registry')
