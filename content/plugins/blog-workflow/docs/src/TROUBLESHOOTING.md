...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin blog-workflow` |
| MCP server timeout | Server not running | Run `just enable-mcp blog-workflow` |
...
2. Install plugin dependencies: `cd content/plugins/blog-workflow && brew bundle`
...
A: Run `just update-plugin blog-workflow` to fetch the latest version.
