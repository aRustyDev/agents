...
|---------|-------|----------|
| Command not found | Plugin not installed | Run `just install-plugin <plugin-name>` |
| MCP server timeout | Server not running | Run `just enable-mcp <plugin-name>` |
...
2. Install plugin dependencies: `cd content/plugins/<plugin-name> && brew bundle`
3. Retry installation: `just install-plugin <plugin-name>`
...
2. Restart the server: `just restart-mcp <plugin-name>`
...
A: Run `just update-plugin <plugin-name>` to fetch the latest version.
