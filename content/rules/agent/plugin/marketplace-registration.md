...
- When manually creating a plugin in `content/plugins/<name>/`
- When restoring or migrating plugins from another source

## Registration Steps

1. Read the plugin's manifest: `content/plugins/<name>/.claude-plugin/plugin.json`
...
| `source` | `./content/plugins/<name>` | Yes |
...
{
  "name": "terraform-dev",
  "source": "./content/plugins/terraform-dev",
...
- `content/plugins/<name>/.claude-plugin/plugin.json` - Individual plugin manifests
