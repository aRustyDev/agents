...
4. Check if `content/plugins/<plugin-name>/` already exists — ask to overwrite or abort

### Step 2: Copy Template

```bash
cp -r content/plugins/.template/ content/plugins/<plugin-name>/
```

### Step 3: Generate plugin.json

Read the template `content/plugins/.template/.claude-plugin/plugin.json` and populate:
...
Write to `content/plugins/<plugin-name>/.claude-plugin/plugin.json`.
...
Write to `content/plugins/<plugin-name>/.claude-plugin/plugin.sources.json`.
...
Write to `content/plugins/<plugin-name>/.mcp.json`.
...
Write to `content/plugins/<plugin-name>/.lsp.json`.
...
Write to `content/plugins/<plugin-name>/brewfile`.
...
Write to `content/plugins/<plugin-name>/README.md`.
