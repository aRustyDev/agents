...
Search `context/plugins/` for plugins in the same domain:

```text
Glob: context/plugins/*/README.md
Glob: context/plugins/*/.claude-plugin/plugin.json
```
...
Search `context/commands/` for matching commands:

```text
Grep: context/commands/*.md for relevant keywords
```

#### Agents (sequential, local only)

Search `context/agents/` for matching agents:

```text
Grep: context/agents/*.md for relevant keywords
```

#### Output Styles (sequential, local only)

Search `context/output-styles/` for matching styles:

```text
Glob: context/output-styles/*.md
```

#### Hooks (sequential, local only)

Search `context/hooks/` for matching hooks:
