...
class TreeNode:
    ...
    @property
    def id(self) -> int:
        """Unique node identifier (proxy to raw node id)."""
        if self._raw is not None:
            return self._raw.id
        return id(self)  # Fallback to Python object id

    @property
    def start_byte(self) -> int:
        """Start byte offset (proxy to span.start_byte)."""
        return self.span.start_byte

    @property
    def end_byte(self) -> int:
        """End byte offset (proxy to span.end_byte)."""
        return self.span.end_byte

    @property
    def start_point(self) -> tuple[int, int]:
        """Start (row, column) position (proxy to span.start_point)."""
        return self.span.start_point

    @property
    def end_point(self) -> tuple[int, int]:
        """End (row, column) position (proxy to span.end_point)."""
        return self.span.end_point
