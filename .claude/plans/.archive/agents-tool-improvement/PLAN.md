...
> **Status: COMPLETE** — All 5 phases implemented and committed prior to 2026-03-20. Superseded by catalog-module-integration plan (also complete).
...
|----|------|--------|--------------|------------------|
| phase-1 | Error Log Separation | **done** | - | Error log exists, catalog entries have no error fields, merge routes errors to log |
| phase-2 | Mechanical Compute Functions | **done** | - | 5 native TS functions, tests passing, `DownloadResult` carries computed fields |
| phase-3 | Incremental Skip + Retry | **done** | phase-1 | `filterForProcessing` replaces `filterAvailable`, `--retry-errors`/`--force` flags work |
| phase-4 | Agent Prompt + Orchestrator Integration | **done** | phase-1, phase-2 | Agent prompt reduced to 5 judgment steps, `--allowedTools` is `Read Glob Grep` |
| phase-5 | Migration + Validation | **done** | phase-1, phase-2, phase-3, phase-4 | `catalog scrub` backfills existing data, test batch produces clean results |
