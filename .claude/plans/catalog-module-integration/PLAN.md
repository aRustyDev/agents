...
|-------|------|-------------|--------|
| 0 | GitHub token provider refactor (keychain + mutex) | None | Medium |
| 1 | Source validation + SSH clone URL support | None | Small |
| 2 | Download orchestrator with `git.ts` + `skill-discovery.ts` | Phase 1 | Medium |
| 3 | Wire new download path (blue/green) + `--git-protocol` flag | Phases 1, 2 | Medium |
| 3b | Remove legacy download path | Phase 3 validated (beads tracked) | Small |
| 4 | Stale detection with `skill-outdated.ts` | Phases 0, 3 | Small |
