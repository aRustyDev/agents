...
(() => {
	...
	MV.parseMatrix = (raw) => {
		...
		while (i < lines.length) {
			...
			if (headingMatch) {
				...
				var section = MV.detectSection(headingText);
			...
			if (
				...
				var tierLevel = parseInt(currentSection.replace("tier", ""), 10);
				var tier = parseTierTable(lines, i, tierLevel);
				// Find or create this tier
				var existing = result.tiers.find((t) => t.level === tierLevel);
				if (existing) {
	...
	MV.detectSection = (heading) => {
		...
		if (lower.includes("runtime recovery") || lower.includes("recovery"))
			return "recovery";
	...
	function parseContextBlock(lines, startIndex) {
		...
		while (i < lines.length && !lines[i].startsWith("#")) {
			...
			var goalMatch = line.match(/\*?\*?Goal\*?\*?:\s*(.+)/i);
			var typeMatch = line.match(/\*?\*?Type\*?\*?:\s*(.+)/i);
			var domainMatch = line.match(/\*?\*?Domain\*?\*?:\s*(.+)/i);
